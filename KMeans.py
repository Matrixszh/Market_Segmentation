...
def plot_kmeans(df3):
    ...
    n = 0 
    X_age_spending = df3[['Age' , 'Spending Score (1-100)']].iloc[: , :].values # extracts only age and spending score information from the dataframe
    inertia = []
    ...
    plt.scatter( x = 'Age' ,y = 'Spending Score (1-100)' , data = df3 , c = labels1 , 
                s = 200 )
    plt.scatter(x = centroids1[: , 0] , y =  centroids1[: , 1] , s = 300 , c = 'red' , alpha = 0.5)
    plt.ylabel('Spending Score (1-100)') , plt.xlabel('Age')
    plt.show()

df3 = pd.read_csv(r'C:\Users\hp\OneDrive\Desktop\Mini-Project-main\clustered_customers.csv')
df3.head()
